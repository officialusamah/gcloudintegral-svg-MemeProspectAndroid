...
def _diag_slot(scanner) -> bool:
    """Sample BELOW-FILTER diagnostics so Railway logs are not flooded."""
...
def _cache_pumpportal_watch_payload(scanner, mint: str, payload: dict) -> None:
    """Keep PumpPortal creation fields needed by the 0-30s WATCH layer."""
...
async def _pumpportal_creation_snapshot(scanner, address: str, listed_ts: int):
    """
    WATCH-only fallback from PumpPortal's creation event.

    This is only an early activity hint. It never bypasses the actual BUY
    safety, mint/freeze, Jupiter BUY/reverse-SELL or manual approval gates.
    """
...
async def _fixed_watch_alert(scanner, address: str, listed_ts: int) -> None:
    ...
    while True:
        ...
        # Primary source: exact Pump.fun on-chain TradeEvent snapshot.
        try:
        ...
        # Fallback: PumpPortal creation payload.
        try:
        ...
        if portal_snap is not None:
            ...
            max_liquidity = max(max_liquidity, portal_liquidity)
    ...
    # Important diagnostic: if this appears, the $100 activity gate passed.
    print(
        f"WATCH QUALIFIED — {_short(address)} — age {age}s — "
        f"volume ${volume:,.0f} — liquidity ${liquidity:,.0f} — "
        f"source {source_label or 'unknown'} — eligible_users {len(users)}",
        flush=True,
    )
    ...
    if sent:
        print(
            f"ULTRA WATCH SENT — {_short(address)} — "
...
def install() -> None:
    ...
    # launch_first's wrapper looks up this module global at runtime.
    ...
    print(
        ...
        f"WATCH diagnostics ON (sampled up to {diag_limit}/min below-filter); "
