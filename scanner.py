...
class Scanner:
    def __init__(self, cfg: Config, store: Store, market: MarketProviders, telegram: Telegram, accounts):
        self.cfg = cfg
        self.store = store
        self.market = market
        self.telegram = telegram
        self.accounts = accounts
        self.live = LiveTrader(cfg, store, market, telegram, accounts)
    ...
    async def run_once(self, send_alerts: bool = True) -> list[ScanItem]:
        ...
        async with self.lock:
            ...
            for candidate, snap in enriched:
                ...
                # Paper engine is disabled in v1.9.1. Keep this call harmless for
                # compatibility if the setting is ever intentionally re-enabled.
                ...
                should_alert = (
                    send_alerts
                    and bool(self.accounts.active_users())
                ...
                if should_alert:
                    ...
                    try:
                        ...
                        sent = 0
                        for user in self.accounts.active_users():
                            try:
                                await self.telegram.send(message, str(user["chat_id"]))
                                sent += 1
                            except Exception as e:
                                print("Telegram subscriber alert failed:", repr(e))
                        if sent:
            ...
            # IMPORTANT: live signals are evaluated from strongest to weakest.
            # With one allowed live position, the highest-scoring eligible token
            # gets the first chance at the slot instead of whichever API item
            # happened to be processed first.
            for item in ordered:
                live_signal_closed = await self.live.manage(item.snapshot, item.result)
                if live_signal_closed:
                    self.fast_rescan_requested = True

            if self.fast_rescan_requested:
                self.fast_rescan_requested = False
                asyncio.create_task(self._fast_rescan_soon())
            return ordered
    ...
    async def loop(self):
        while True:
            ...
            # Faster alert cycle for BUY/SELL decisions; default 30s.
            # This changes only monitoring/alerts, never wallet signing.
            await asyncio.sleep(self.cfg.fast_alert_scan_seconds)
