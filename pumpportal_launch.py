...
def _cache_launch_metadata(scanner, mint: str, payload: dict) -> None:
    """Cache PumpPortal's creation metadata for the narrative engine."""
    cache = getattr(scanner, "_narrative_launch_meta", None)
    if cache is None:
        cache = {}
        scanner._narrative_launch_meta = cache
        ...
        try:
            async with websockets.connect(
                ...
                async for raw in ws:
                    ...
                    # Save name/symbol BEFORE address de-duplication. Helius may have
                    # detected the same mint first; its narrative task can still use
                    # this PumpPortal metadata without making another Helius call.
                    _cache_launch_metadata(scanner, mint, payload)
