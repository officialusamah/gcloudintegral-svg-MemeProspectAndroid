...
/livestatus — live signal + execution safety status
/livepositions — open live-market signal positions
/starttrading CONFIRM — unavailable (manual live signals only)
            ...
            for u in updates:
                ...
                if cmd == "/help":
                    ...
                    await tg.send(
                        ...
                        "ℹ️ The bot watches real market data and sends manual trade signals. "
                        "It does not submit transactions or move wallet funds."
                    )

                elif cmd == "/livepositions":
                    rows = scanner.live.open_positions()
                    if not rows:
                        await tg.send("<b>📡 Live signal positions</b>\nNo open live-market signal positions.")
                    else:
                        lines = ["<b>📡 Open LIVE SIGNAL positions</b>"]
                        for i, p in enumerate(rows, 1):
                    ...
                    await tg.send(
                        ...
                        "The live bot is operating in manual signal mode:\n"
                        f"• BUY signal at {cfg.live_signal_buy_score}/100+\n"
                        f"• SELL ALL signal at +{cfg.live_signal_take_profit_percent:.0f}%\n"
                        "• Immediate fresh scan after the exit signal\n\n"
                        "No wallet transaction is submitted automatically."
                    )
