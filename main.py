    ...
    while True:
        try:
            ...
            for u in updates:
                ...
                # Register/update user metadata on every command.
                referral = ""
                if cmd == "/start" and len(parts) > 1 and parts[1].lower().startswith("ref_"):
                    referral = parts[1][4:].upper()
                user = accounts.register(
                    chat_id, username, first_name, referral_code=referral
                )
                tg.chat_id = accounts.admin_chat_id() or tg.chat_id

                if cmd == "/start":
                    role = "\n\n👑 <b>You are the bot owner/admin.</b>" if accounts.is_admin(chat_id) else ""
                    await tg.send(
                ...
                if cmd == "/help":
                    ...
                    await tg.send(subscription_text(accounts, chat_id, cfg), chat_id)
                    ...
                    if not wallet:
                        ...
                        try:
                            sol = await market.solana_wallet_balance(wallet, cfg.solana_rpc_url)
                            await tg.send(
                    ...
                    if not rows:
                        ...
                        for p in rows:
                            ...
                            state = (
                                ...
                                if exiting else
                                ("🟢 BUY CONFIRMED / MONITORING" if confirmed
                            ...
                            ret = ((last / entry) - 1) * 100 if confirmed and entry and last else 0
                            lines.append(
                                ...
                                + (
                                    ...
                                    if confirmed else
                                    "P/L starts after on-chain BUY confirmation."
                                )
                            )
                        await tg.send("\n".join(lines), chat_id)
                    ...
                    if not rows:
                        ...
                        lines = ["<b>📒 Your recent wallet-tracked trades</b>"]
                        for p in rows:
                            ret = p.get("realized_return_pct")
                            ret_text = "n/a" if ret is None else f"{float(ret):+.1f}%"
                            lines.append(
                                f"\n<b>{html.escape(str(p.get('token_symbol') or '?'))}</b> — "
                                f"<b>{ret_text}</b>\n"
                    ...
                    if not accounts.is_active(chat_id):
                        ...
                        await tg.send(format_top(store.latest_top(5)), chat_id)

                elif cmd == "/starttrading":
                    user = accounts.get(chat_id) or {}
                    if not accounts.is_active(chat_id):
                        await tg.send("💎 Your Pro subscription is inactive. Use /plans.", chat_id)
                        ...
                        await tg.send(
                            ...
                            "Existing and future wallet-tracked positions use manual Solflare approval.",
                            chat_id,
                        )
                    ...
                    if not accounts.is_admin(chat_id):
                        continue
                    await tg.send("🔎 Running a scan now…", chat_id)
                    ...
                    for urow in rows:
                        ...
                        name = str(urow.get("username") or urow.get("first_name") or "user")
                        active = accounts.is_active(cid)
                        wallet = "👛" if str(urow.get("wallet_address") or "") else "—"
                        role = "👑" if int(urow.get("is_admin") or 0) else ""
                        lines.append(
                            f"\n{role}<b>{html.escape(name)}</b> {wallet}\n"
                    ...
                    if len(parts) < 3:
                        await tg.send(
                            ...
                            "Example: <code>/activate 123456789 30 15000</code>",
                            chat_id,
                        )
                        continue
                    target = parts[1]
                    try:
                        days = int(parts[2])
                        amount = float(parts[3].replace(",", "")) if len(parts) > 3 else 0
                    except ValueError:
                        await tg.send("Days and amount must be numbers.", chat_id)
                        continue
                    accounts.activate(target, days, amount, "Manual admin activation")
                    await tg.send(
                        f"✅ Activated <code>{html.escape(target)}</code> for <b>{days} days</b>"
                        + (f" — ₦{amount:,.0f} recorded" if amount else ""),
                        chat_id,
                    )
                    try:
                        await tg.send(
                            f"✅ <b>Meme Prospect Pro activated.</b>\n"
                            f"Access: <b>{days} days</b>\n"
                            "Connect your public wallet with /setwallet and use /starttrading.",
                            target,
                        )
                    except Exception:
                        pass
                    ...
                    if not accounts.is_admin(chat_id):
                        continue
                    if len(parts) != 2:
                        await tg.send("Use: <code>/deactivate CHAT_ID</code>", chat_id)
                        continue
                    target = parts[1]
                    accounts.deactivate(target)
                    await tg.send(
                        f"🛑 Deactivated <code>{html.escape(target)}</code>.",
                        chat_id,
                    )
                    ...
                    if not body:
                        await tg.send("Use: <code>/broadcast YOUR MESSAGE</code>", chat_id)
                        continue
                    sent = 0
                    for target_user in accounts.active_users():
                        try:
                            ...
                            pass
                    await tg.send(f"✅ Broadcast sent to <b>{sent}</b> active users.", chat_id)
...
async def main():
    ...
    tg = Telegram(
        ...
        store,
        accounts.admin_chat_id() or cfg.admin_telegram_chat_id,
    )
    scanner = Scanner(cfg, store, market, tg, accounts)
    ...
    tasks = [
        ...
        asyncio.create_task(command_loop(tg, scanner, cfg, store, market, accounts)),
    ]
    try:
