...
ULTRA_RETRY_SECONDS = max(15, _int("ULTRA_EARLY_RETRY_SECONDS", 60))
...
ULTRA_REST_POLL_SECONDS = max(3.0, _float("ULTRA_EARLY_REST_POLL_SECONDS", 5.0))
ULTRA_DEX_MAX_CONCURRENCY = max(1, min(10, _int("ULTRA_EARLY_DEX_MAX_CONCURRENCY", 4)))
ULTRA_DEX_RETRY_SECONDS = max(1.0, _float("ULTRA_EARLY_DEX_RETRY_SECONDS", 2.0))
ULTRA_DEX_BACKOFF_SECONDS = max(2.0, _float("ULTRA_EARLY_DEX_BACKOFF_SECONDS", 4.0))
...
def _ultra_dex_semaphore(scanner):
    sem = getattr(scanner, "_ultra_dex_sem", None)
    if sem is None:
        sem = asyncio.Semaphore(ULTRA_DEX_MAX_CONCURRENCY)
        scanner._ultra_dex_sem = sem
    return sem

async def _ultra_pair_snapshot(scanner, candidate):
    """Return (snapshot, reason, backoff_seconds) without flooding DEX Screener."""
    sem = _ultra_dex_semaphore(scanner)
    async with sem:
        try:
            ...
            if status:
                status = int(status)
                reason = f"DEX HTTP {status}"
                # 429 = rate limit; 5xx = temporary upstream failure.
                backoff = ULTRA_DEX_BACKOFF_SECONDS if status == 429 or status >= 500 else ULTRA_DEX_RETRY_SECONDS
                return None, reason, backoff
...
async def _ultra_rest_fallback(scanner) -> None:
    ...
    while True:
        try:
            ...
            batch = await scanner.market._birdeye_new_listings("solana")
            addresses = [
...
def install() -> None:
    ...
    def patched_scanner_init(self, *args, **kwargs):
        ...
        self._ultra_dex_sem = asyncio.Semaphore(ULTRA_DEX_MAX_CONCURRENCY)
    ...
    async def patched_loop(self):
        print(
            f"ULTRA DEX throttle ON — max_concurrency={ULTRA_DEX_MAX_CONCURRENCY} "
            f"retry={ULTRA_DEX_RETRY_SECONDS:.0f}s backoff={ULTRA_DEX_BACKOFF_SECONDS:.0f}s",
            flush=True,
        )
