...
async def _handle_ultra_listing(scanner, address: str, listed_ts: int) -> None:
    ...
    while time.time() - started <= ULTRA_RETRY_SECONDS:
        ...
        checks = [
            ...
            (snap.market_cap >= ACTION_BUY_MIN_MCAP,
             f"market cap ${snap.market_cap:,.0f} < entry band ${ACTION_BUY_MIN_MCAP:,.0f}"),
            (snap.market_cap <= ACTION_BUY_MAX_MCAP,
             f"market cap ${snap.market_cap:,.0f} > entry band ${ACTION_BUY_MAX_MCAP:,.0f}"),
...
def install() -> None:
    ...
    def patched_candidate_ok(self, snap, result):
        ...
        return ACTION_BUY_MIN_MCAP <= mcap <= ACTION_BUY_MAX_MCAP
