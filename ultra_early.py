...
ACTION_BUY_MIN_MCAP = max(1.0, _float("ACTION_BUY_MIN_MARKET_CAP_USD", 20_000))
...
ULTRA_CONFIRMATION_START_SECONDS = max(30, _int("ULTRA_CONFIRMATION_START_SECONDS", 120))
ULTRA_CONFIRMATION_MIN_SCORE = max(ULTRA_MIN_SCORE, min(100, _int("ULTRA_CONFIRMATION_MIN_SCORE", 75)))
...
def _moonshot_override_ok(snap, result) -> bool:
    """Rare high-momentum exception above the normal $20k-$35k BUY band."""
...
async def _handle_ultra_listing(scanner, address: str, listed_ts: int) -> None:
    ...
    while time.time() - started <= ULTRA_RETRY_SECONDS:
        ...
        checks = [
            ...
            (_action_buy_mcap_ok(snap, result),
             f"market cap ${snap.market_cap:,.0f} outside normal ${ACTION_BUY_MIN_MCAP:,.0f}-${ACTION_BUY_MAX_MCAP:,.0f} band and moonshot override not met"),
    ...
    async def patched_telegram_send(self, text, *args, **kwargs):
        ...
        if isinstance(text, str) and text.startswith("<b>🔐 Your Live Bot Status</b>"):
            ...
            current = [
                ...
                "⚡ Ultra Early: <b>0–2 min / 65+ with strict checks</b>",
                "✅ Confirmation zone: <b>2–5 min / 75+ with stricter momentum</b>",
                "💰 Normal BUY market cap: <b>$20,000–$55,000</b>",
