...
ACTION_BUY_MAX_MCAP = max(ACTION_BUY_MIN_MCAP, _float("ACTION_BUY_MAX_MARKET_CAP_USD", 35_000))
...
MOONSHOT_MIN_SCORE = max(90, min(100, _int("MOONSHOT_MIN_SCORE", 95)))
MOONSHOT_MAX_AGE_SECONDS = max(60, _int("MOONSHOT_MAX_AGE_SECONDS", 300))
MOONSHOT_MIN_BUY_RATIO = min(1.0, max(0.60, _float("MOONSHOT_MIN_BUY_RATIO", 0.80)))
...
ULTRA_CONFIRMATION_MIN_BUY_RATIO = min(1.0, max(ULTRA_MIN_BUY_RATIO, _float("ULTRA_CONFIRMATION_MIN_BUY_RATIO", 0.65)))
...
def install() -> None:
    ...
    async def patched_telegram_send(self, text, *args, **kwargs):
        ...
        if isinstance(text, str) and text.startswith("<b>🔐 Your Live Bot Status</b>"):
            ...
            current = [
                "Normal BUY route: <b>75+/100 + entry checks</b>",
                "⚡ Ultra Early: <b>0–2 min / 65+ with strict checks</b>",
                "✅ Confirmation zone: <b>2–5 min / 75+ with stricter momentum</b>",
                "💰 Normal BUY market cap: <b>$20,000–$35,000</b>",
                "🚀 Moonshot override: <b>$35,000–$150,000 / 95+ strict profile</b>",
    ...
    def patched_candidate_ok(self, snap, result):
        # Preserve every existing live-signal rule, then tighten only the
        # actionable BUY market-cap window. Normal prospect/watch alerts can
        # still exist outside this band; BUY NOW cannot.
        if not original_candidate_ok(self, snap, result):
            return False
    ...
    async def patched_loop(self):
        ...
        print(
            ...
            f"${ACTION_BUY_MAX_MCAP:,.0f} (target area ~$25k)",
            flush=True,
        )
        print(
