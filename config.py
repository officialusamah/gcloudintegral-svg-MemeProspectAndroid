...
class Config:
    ...
    # TELEGRAM_CHAT_ID remains a legacy owner/admin fallback.
    admin_telegram_chat_id: str = os.getenv(
        "ADMIN_TELEGRAM_CHAT_ID", os.getenv("TELEGRAM_CHAT_ID", "")
    ).strip()
    bot_username: str = os.getenv("BOT_USERNAME", "Memeprospects_bot").strip().lstrip("@")
    subscription_price_ngn: int = max(0, _int("SUBSCRIPTION_PRICE_NGN", 0))
    subscription_default_days: int = max(1, _int("SUBSCRIPTION_DEFAULT_DAYS", 30))
    birdeye_api_key: str = os.getenv("BIRDEYE_API_KEY", "").strip()

    # Read-only Solana wallet connection.
    wallet_public_address: str = os.getenv("WALLET_PUBLIC_ADDRESS", "").strip()
    ...
    live_signal_buy_score: int = min(100, max(1, _int("LIVE_SIGNAL_BUY_SCORE", 75)))
    ...
    # Read-only on-chain wallet confirmation.
    wallet_trade_tracking_enabled: bool = _bool("WALLET_TRADE_TRACKING_ENABLED", True)
    wallet_buy_confirm_timeout_seconds: int = max(
        60, _int("WALLET_BUY_CONFIRM_TIMEOUT_SECONDS", 300)
    )
    ...
    fast_alert_scan_seconds: int = max(20, _int("FAST_ALERT_SCAN_SECONDS", 30))
    ...
    min_market_cap_usd: float = _float("MIN_MARKET_CAP_USD", 20_000)
    ...
    paper_trade_enabled: bool = False  # v1.9.1: legacy paper trader disabled
