...
class Config:
    ...
    chains: tuple[str, ...] = tuple(
        x.strip().lower() for x in os.getenv("CHAINS", "solana,bsc,base,ethereum").split(",") if x.strip()
    )
    min_liquidity_usd: float = _float("MIN_LIQUIDITY_USD", 10_000)
    min_market_cap_usd: float = _float("MIN_MARKET_CAP_USD", 200_000)
