...
class LiveTrader:
    ...
    async def _send_buy_signal(self, snap, result):
        ...
        await self.telegram.send(
            f"🟢 <b>LIVE BUY SIGNAL — MANUAL ONLY</b>\n\n"
            f"<b>{html.escape(snap.token_name)} "
            f"({html.escape(snap.token_symbol)})</b>\n"
            ...
            "ℹ️ No transaction was submitted. You decide whether to trade manually."
        )
    ...
    async def _send_sell_signal(self, snap, ret: float, reason: str):
        if not self.telegram.chat_id:
            return
        await self.telegram.send(
            f"💰 <b>LIVE SELL ALL SIGNAL — MANUAL ONLY</b>\n\n"
            f"<b>{html.escape(snap.token_symbol)}</b>\n"
            f"Signal return: <b>{ret:+.1f}%</b>\n"
            f"Reason: <b>{html.escape(reason)}</b>\n\n"
            "🔎 Fresh opportunity scan starts immediately.\n"
            "ℹ️ No transaction was submitted."
        )
