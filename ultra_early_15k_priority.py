...
ULTRA_MAX_AGE_SECONDS = max(15, _int("ULTRA_EARLY_MAX_AGE_SECONDS", 300))
ULTRA_RETRY_SECONDS = max(15, _int("ULTRA_EARLY_RETRY_SECONDS", 300))
...
ACTION_BUY_MIN_MCAP = max(1.0, _float("ACTION_BUY_MIN_MARKET_CAP_USD", 15_000))
ACTION_BUY_MAX_MCAP = max(ACTION_BUY_MIN_MCAP, _float("ACTION_BUY_MAX_MARKET_CAP_USD", 55_000))
MOONSHOT_OVERRIDE_ENABLED = _bool("MOONSHOT_OVERRIDE_ENABLED", True)
MOONSHOT_MAX_MCAP = max(ACTION_BUY_MAX_MCAP, _float("MOONSHOT_MAX_MARKET_CAP_USD", 150_000))
MOONSHOT_MIN_SCORE = max(75, min(100, _int("MOONSHOT_MIN_SCORE", 80)))
MOONSHOT_MAX_AGE_SECONDS = max(60, _int("MOONSHOT_MAX_AGE_SECONDS", 300))
MOONSHOT_MIN_BUY_RATIO = min(1.0, max(0.50, _float("MOONSHOT_MIN_BUY_RATIO", 0.55)))
MOONSHOT_MIN_TXNS = max(20, _int("MOONSHOT_MIN_TXNS", 100))
MOONSHOT_MIN_VOLUME_MCAP_RATIO = max(0.05, _float("MOONSHOT_MIN_VOLUME_MCAP_RATIO", 0.35))
MOONSHOT_MIN_LIQUIDITY_MCAP_RATIO = max(0.05, _float("MOONSHOT_MIN_LIQUIDITY_MCAP_RATIO", 0.15))
MOONSHOT_MAX_M5_MOVE = max(10.0, _float("MOONSHOT_MAX_5M_MOVE_PERCENT", 60.0))
STRONG_BREAKOUT_ENABLED = _bool("STRONG_BREAKOUT_ENABLED", True)
STRONG_BREAKOUT_MIN_SCORE = max(90, min(100, _int("STRONG_BREAKOUT_MIN_SCORE", 95)))
STRONG_BREAKOUT_MAX_AGE_SECONDS = max(300, _int("STRONG_BREAKOUT_MAX_AGE_SECONDS", 1200))
STRONG_BREAKOUT_MAX_MCAP = max(MOONSHOT_MAX_MCAP, _float("STRONG_BREAKOUT_MAX_MARKET_CAP_USD", 250_000))
STRONG_BREAKOUT_MIN_BUY_RATIO = min(1.0, max(0.55, _float("STRONG_BREAKOUT_MIN_BUY_RATIO", 0.65)))
STRONG_BREAKOUT_MIN_TXNS = max(100, _int("STRONG_BREAKOUT_MIN_TXNS", 300))
STRONG_BREAKOUT_MIN_VOLUME_MCAP_RATIO = max(0.20, _float("STRONG_BREAKOUT_MIN_VOLUME_MCAP_RATIO", 0.30))
STRONG_BREAKOUT_MIN_LIQUIDITY_MCAP_RATIO = max(0.10, _float("STRONG_BREAKOUT_MIN_LIQUIDITY_MCAP_RATIO", 0.15))
STRONG_BREAKOUT_MAX_M5_MOVE = max(10.0, _float("STRONG_BREAKOUT_MAX_5M_MOVE_PERCENT", 40.0))
ULTRA_MIN_TXNS = max(1, _int("ULTRA_EARLY_MIN_TXNS", 8))
ULTRA_MIN_BUY_RATIO = min(1.0, max(0.50, _float("ULTRA_EARLY_MIN_BUY_RATIO", 0.55)))
...
ULTRA_CONFIRMATION_START_SECONDS = max(180, _int("ULTRA_CONFIRMATION_START_SECONDS", 180))
ULTRA_CONFIRMATION_MIN_SCORE = max(ULTRA_MIN_SCORE, min(100, _int("ULTRA_CONFIRMATION_MIN_SCORE", 65)))
ULTRA_CONFIRMATION_MIN_BUY_RATIO = min(1.0, max(ULTRA_MIN_BUY_RATIO, _float("ULTRA_CONFIRMATION_MIN_BUY_RATIO", 0.55)))
ULTRA_CONFIRMATION_MAX_M5_MOVE = max(10.0, _float("ULTRA_CONFIRMATION_MAX_5M_MOVE_PERCENT", 60.0))
...
def _pair_age_seconds(snap) -> int:
    try:
...
def _moonshot_override_ok(snap, result) -> bool:
    """Rare high-momentum exception above the normal $15k-$55k BUY band."""
    if not MOONSHOT_OVERRIDE_ENABLED:
        return False
    mcap = float(getattr(snap, "market_cap", 0) or 0)
    if not (ACTION_BUY_MAX_MCAP < mcap <= MOONSHOT_MAX_MCAP):
        return False
    if bool(getattr(result, "hard_block", False)) or int(getattr(result, "score", 0) or 0) < MOONSHOT_MIN_SCORE:
        return False
    ...
    move = float(getattr(snap, "price_change_m5", 0) or 0)

    return (
        _pair_age_seconds(snap) <= MOONSHOT_MAX_AGE_SECONDS
        and total >= MOONSHOT_MIN_TXNS
        and buy_ratio >= MOONSHOT_MIN_BUY_RATIO
        and (volume / mcap if mcap > 0 else 0) >= MOONSHOT_MIN_VOLUME_MCAP_RATIO
        and (liquidity / mcap if mcap > 0 else 0) >= MOONSHOT_MIN_LIQUIDITY_MCAP_RATIO
        and 0 < move <= MOONSHOT_MAX_M5_MOVE
    )


def _strong_breakout_ok(snap, result) -> bool:
    """
    High-quality exception for a token that has moved beyond the normal
    5-minute / $150k Moonshot window but is still showing unusually strong,
    liquid, active momentum. It never bypasses security or Jupiter sellability.
    """
    if not STRONG_BREAKOUT_ENABLED:
        return False

    mcap = float(getattr(snap, "market_cap", 0) or 0)
    if not (MOONSHOT_MAX_MCAP < mcap <= STRONG_BREAKOUT_MAX_MCAP):
        return False

    score = int(getattr(result, "score", 0) or 0)
    if bool(getattr(result, "hard_block", False)) or score < STRONG_BREAKOUT_MIN_SCORE:
        return False
    ...
    return (
        _pair_age_seconds(snap) <= STRONG_BREAKOUT_MAX_AGE_SECONDS
        and total >= STRONG_BREAKOUT_MIN_TXNS
        and buy_ratio >= STRONG_BREAKOUT_MIN_BUY_RATIO
        and (volume / mcap if mcap > 0 else 0) >= STRONG_BREAKOUT_MIN_VOLUME_MCAP_RATIO
        and (liquidity / mcap if mcap > 0 else 0) >= STRONG_BREAKOUT_MIN_LIQUIDITY_MCAP_RATIO
        and 0 < move <= STRONG_BREAKOUT_MAX_M5_MOVE
    )


def _action_buy_mcap_ok(snap, result) -> bool:
    mcap = float(getattr(snap, "market_cap", 0) or 0)
    return (
        ACTION_BUY_MIN_MCAP <= mcap <= ACTION_BUY_MAX_MCAP
        or _moonshot_override_ok(snap, result)
        or _strong_breakout_ok(snap, result)
    )
...
async def _ultra_send_buy(scanner, user: dict, snap, result, age_seconds: int) -> None:
    ...
    moonshot = _moonshot_override_ok(snap, result)
    headline = "🚀 MOONSHOT OVERRIDE — ACTION REQUIRED" if moonshot else "⚡🚨 ULTRA EARLY BUY SETUP 🚨⚡"
...
async def _handle_ultra_listing(scanner, address: str, listed_ts: int) -> None:
    ...
    while time.time() - started <= ULTRA_RETRY_SECONDS:
        ...
        if is_pump_onchain:
            # Do not hard-block a brand-new Pump.fun token just because the
            # bonding curve has not yet accumulated 1 SOL. For the user's
            # early-entry strategy, reserve depth remains an informational/
            # scoring signal, while actual tradability is enforced later by
            # the mandatory Jupiter BUY + reverse-SELL checks.
            depth_check = (True, "")
        else:
            depth_check = (
...
def install() -> None:
    ...
    original_candidate_ok = LiveTrader._candidate_ok
    original_telegram_send = Telegram.send

    async def patched_telegram_send(self, text, *args, **kwargs):
        # Display-only compatibility patch: main.py still prints legacy
        # 3-minute/60+ Ultra status values. Rewrite only the /livestatus
        # response so Telegram reflects the actual deployed strategy.
        if isinstance(text, str) and text.startswith("<b>🔐 Your Live Bot Status</b>"):
            lines = text.split("\n")
            kept = []
            for line in lines:
                if line.startswith("Normal BUY route:"):
                    continue
                if line.startswith("Ultra Early route:"):
                    continue
                kept.append(line)

            insert_at = 5 if len(kept) >= 5 else len(kept)
            current = [
                "Normal BUY route: <b>75+/100 + early-entry checks</b>",
                "⚡ Ultra Early: <b>0–3 min / 65+ with strict checks</b>",
                "✅ Confirmation zone: <b>3–5 min / 75+ with stricter momentum</b>",
                "💰 Normal BUY market cap: <b>$10,000–$55,000</b>",
                "🚀 Moonshot override: <b>$55,000–$150,000 / 80+ strong profile</b>\n🔥 Strong breakout: <b>$150,000–$250,000 / 95+ / up to 20 min</b>",
                "🔐 Execution: <b>manual wallet approval only</b>",
            ]
            kept[insert_at:insert_at] = current
            text = "\n".join(kept)

        return await original_telegram_send(self, text, *args, **kwargs)
    ...
    def patched_candidate_ok(self, snap, result):
        # Early-exit profile: allow smaller but still meaningful liquidity so
        # the bot can enter earlier, while keeping score, chain, momentum,
        # market-cap and all downstream security/Jupiter sellability checks.
        if not self.cfg.live_signal_enabled:
            return False
        if getattr(snap, "chain", "") != "solana":
            return False
        if bool(getattr(result, "hard_block", False)):
            return False
        if int(getattr(result, "score", 0) or 0) < int(self.cfg.live_signal_buy_score):
            return False
        if float(getattr(snap, "price_usd", 0) or 0) <= 0:
            return False
        if float(getattr(snap, "liquidity_usd", 0) or 0) < 10_000:
            return False
        if float(getattr(snap, "market_cap", 0) or 0) <= 0:
            return False
        if float(getattr(snap, "price_change_m5", 0) or 0) > float(self.cfg.live_max_5m_move_percent):
            return False
        return _action_buy_mcap_ok(snap, result)
    ...
    async def patched_loop(self):
        ...
        print(
            f"MOONSHOT override — up to ${MOONSHOT_MAX_MCAP:,.0f}; "
            f"score {MOONSHOT_MIN_SCORE}+, age <= {MOONSHOT_MAX_AGE_SECONDS}s, "
            f"buys >= {MOONSHOT_MIN_BUY_RATIO*100:.0f}%, txns >= {MOONSHOT_MIN_TXNS}, "
            f"vol/mcap >= {MOONSHOT_MIN_VOLUME_MCAP_RATIO:.2f}",
            flush=True,
        )
        print(
            f"ACTION BUY market-cap band — ${ACTION_BUY_MIN_MCAP:,.0f}–"
            f"${ACTION_BUY_MAX_MCAP:,.0f} (early-entry zone)",
            flush=True,
        )
        print(
            f"ULTRA timing — 0-{ULTRA_CONFIRMATION_START_SECONDS}s ultra zone; "
            f"{ULTRA_CONFIRMATION_START_SECONDS + 1}-{ULTRA_MAX_AGE_SECONDS}s confirmation zone "
            f"(score {ULTRA_CONFIRMATION_MIN_SCORE}+, max 5m move "
            f"{ULTRA_CONFIRMATION_MAX_M5_MOVE:.0f}%)",
            flush=True,
        )
